...
class TemplatesController < ApplicationController
  ...
  def build_js_module(templates)
    ...
    output << "}"
    output << ""
    output << "// Path helper stubs for generating URLs (used by heatlist navigation links)"
    output << "// These links are intercepted by SPA navigation, so hardcoded paths are fine"
    output << "// Action URLs (POST targets, data-* attributes) use server-computed paths from data.paths"
