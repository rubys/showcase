...
Events can have one or more displays showing the current heat number.
  ...
  * Use left/right arrow keys to cycle through four display layouts:
      * **number** (default): giant heat number only
      * **stacked**: heat number, dance name, and participant names in a vertical list
      * **split**: number and dance on the left, names on the right
      * **columns**: number and dance on top, names in two columns below
  * The chosen layout is saved in the browser and persists across refreshes.  You can also link directly to a layout with `?layout=stacked`, `?layout=split`, or `?layout=columns`.
