...
class ScoresControllerTest < ActionDispatch::IntegrationTest
  ...
  test "agenda traversal" do
    ...
    assert_select 'a[rel=next][href=?]', judge_heat_path(judge: judge, heat: 2, style: 'radio', sort: 'back')

    get judge_heat_url(judge: judge, heat: 3)
    assert_select 'a[rel=prev][href=?]', judge_heat_path(judge: judge, heat: 2, style: 'radio', sort: 'back')
    assert_select 'a[rel=next][href=?]', judge_heat_slot_path(judge: judge, heat: 4, slot: 1, style: 'radio', sort: 'back')

    get judge_heat_slot_url(judge: judge, heat: 4, slot: 1)
    assert_response :success
    assert_select 'a[rel=prev][href=?]', judge_heat_path(judge: judge, heat: 3, style: 'radio', sort: 'back')
    assert_select 'a[rel=next][href=?]', judge_heat_slot_path(judge: judge, heat: 4, slot: 2, style: 'radio', sort: 'back')

    get judge_heat_slot_url(judge: judge, heat: 4, slot: 2)
    assert_response :success
    assert_select 'a[rel=prev][href=?]', judge_heat_slot_path(judge: judge, heat: 4, slot: 1, style: 'radio', sort: 'back')
    assert_select 'a[rel=next][href=?]', judge_heat_path(judge: judge, heat: 5, style: 'radio', sort: 'back')

    get judge_heat_url(judge: judge, heat: 5)
    assert_response :success
    assert_select 'a[rel=prev][href=?]', judge_heat_slot_path(judge: judge, heat: 4, slot: 2, style: 'radio', sort: 'back')
